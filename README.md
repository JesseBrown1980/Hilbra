...
- 🟡 **IN PROGRESS (Rust canonical node):** the Host‑8 workspace now **compiles green on a real Rust 1.81 toolchain** (`cargo check`/`clippy --workspace` in CI), and a read‑only `host8-serve` runs locally; the BEHCS‑1024 **kernel boot on metal** remains future (G1). Recall is now served by an **in‑memory inverted index** (`HILBRA-IDX-BEHCS-TUPLE-TEXT-V1`), MEASURED at ~60 ms (Node) on the 591k‑row corpus. A Rust drop‑in recall engine reproduces the index (2–16 ms, L0 measured PII‑free) but is **HELD under bilateral review** for *robustness/parity* hardening (PR #8 on `asolaria-federation-1024`: authorization‑grant generalization, fail‑closed‑corpus, dup‑ordering, bounded concurrency). The PII path‑vs‑content item is **non‑blocking** defense‑in‑depth (L0 is measured PII‑free; keyed acer↔liris PII is intended). — **not release‑ready; no cutover; Node stays live.**
